...
class User
  ...
  def requirements_met?(requirements)
    ...
    requirements.each do |type, req|
      ...
      when 'rooms'
        req.each do |room_id, count|
          own_count = self.rooms.where(reference_id: room_id).count
          raise 'not enough required rooms: ' + room_id if own_count < count.to_i
        end
      when 'level'
  ...
  def give_rewards(rewards)
    ...
    rewards.each do |type, rew|
      ...
      when 'experience'
        self.experience += rew.to_i
      when 'reputation'
        self.reputation += rew.to_i
